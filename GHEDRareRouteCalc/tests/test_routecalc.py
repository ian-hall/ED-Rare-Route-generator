...
#------------------------------------------------------------------------------
    def setUp(self):
        ...
        RouteCalc.Set_Valid_Systems(self.All_Systems)
...
#------------------------------------------------------------------------------
    def test_Selection_Vals(self):
        ...
        '''
        selectVals = RouteCalc.Wrap_Calc_Relative_Fitness(self.Population)
...
#------------------------------------------------------------------------------
    def test_Mutate_Special(self):
        ...
        RouteCalc.Set_Valid_Systems(tempValidSystems);
...
#------------------------------------------------------------------------------
    ...
    def test_Start_No_Args(self):
        RouteCalc.Start_Genetic_Solver()
#------------------------------------------------------------------------------
    def test_Start_Bad_Args(self):
        #Assert empty system list raises exception
        with self.assertRaises(Exception):
            RouteCalc.Start_Genetic_Solver(500,[],20,True,self.Fit_Type)

        #Assert passing a pop size under 3 raises exception
        for popSize in range(3):
            with self.subTest(popSize=popSize):
                with self.assertRaises(Exception):
                    RouteCalc.Start_Genetic_Solver(popSize,self.All_Systems,6,True,self.Fit_Type)

        #Assert fail if there are more systems in the route than in the list of valid systems
        with self.assertRaises(Exception):
            RouteCalc.Start_Genetic_Solver(200,self.All_Systems[:self.Route_Length-1],self.Route_Length,True,self.Fit_Type)
#------------------------------------------------------------------------------
    def test_Start_Bad_Routelength(self):
        ...
        for routeLen in range(3):
            with self.subTest(routeLen=routeLen):
                with self.assertRaises(Exception):
                    RouteCalc.Start_Genetic_Solver(self.Pop_Size,self.All_Systems,routeLen,True,FitnessType.EvenSplit)
        ...
        for routeLen in range(6):
            with self.subTest(routeLen=routeLen):
                with self.assertRaises(Exception):
                    RouteCalc.Start_Genetic_Solver(self.Pop_Size,self.All_Systems,routeLen,True,FitnessType.FirstOver)
